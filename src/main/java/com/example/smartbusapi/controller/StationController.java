...
//        List<String> stationNos = Arrays.asList("12114", "12113", "12111", "", "12109", "12107");
        for (String searchNm : searchNms) {
            ...
            urlBuilder.append("&" + URLEncoder.encode("cityCode", "UTF-8") + "=" + URLEncoder.encode("37100", "UTF-8")); /*도시코드*/
                        ...
                        if (item != null) {
                            if (item.isArray() && item.size() > 0){
                                for (int i = 0; i < item.size(); i++) {
                                    ...
                                    processStation(curItem, station, stationNms);}
                            } else{
                                processStation(item, station, stationNms);}
                        }
                    }
                }
            }
    ...
    // 정류소 DB저장을 처리하는 함수
    private void processStation (JsonNode item, Station station, List<String> stationNms) throws ExecutionException, InterruptedException {
